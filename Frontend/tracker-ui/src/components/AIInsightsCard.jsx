    ...
    <div className="glass-card overflow-hidden">
      ...
      <div className="p-4 space-y-3">
        {Array.isArray(insights) && insights.map((item, i) => {
          ...
          const cls = config.cls || 'border-secondary bg-secondary/5';
          const iconCls = config.iconCls || 'text-secondary';
          return (
            <div key={i} className={`p-3 rounded-xl border-l-4 ${cls} flex gap-3 group transition-all hover:translate-x-1`}>
              <Icon className={`w-4 h-4 mt-0.5 shrink-0 ${iconCls}`} />
              <div className="text-xs text-on-surface-variant leading-relaxed font-medium" dangerouslySetInnerHTML={{ __html: item.text || item.content || '' }} />
            </div>
          );
        })}

...
export default function AIInsights() {
  ...
  const handleReAnalyze = async () => {
    try {
      ...
      // Even if endpoint fails, try to refresh stored insights
      await fetchInsights();
    } finally {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-8 rounded-3xl bg-gradient-to-br from-primary/10 via-surface-container to-transparent border border-outline-variant relative overflow-hidden group">
        ...
        <div className="flex gap-3 relative z-10 flex-wrap">
          ...
          <button
            ...
            className="px-5 py-3 bg-primary/10 hover:bg-primary/20 border border-primary/30 rounded-2xl flex items-center gap-2 text-sm font-bold text-primary transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {reanalyzing ? (
            ...
            {reanalyzing ? 'Analyzing...' : 'Re-Analyze Workouts'}
          </button>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              (dbInsights.length === 0 ? [
                { id: 'st-1', type: 'progressive_overload', content: 'Welcome to FitTrack! Target squatting 45kg for 3 sets of 10 reps to build baseline strength.', created_at: new Date().toISOString(), is_read: false },
                { id: 'st-2', type: 'recovery', content: 'Sleep at least 8 hours to allow CNS (Central Nervous System) recovery and optimize muscle repair.', created_at: new Date().toISOString(), is_read: false },
                { id: 'st-3', type: 'imbalance', content: 'Balance press movements (Pushups) with pulling movements (Rows) to protect shoulder joints.', created_at: new Date().toISOString(), is_read: false }
              ] : dbInsights).map((item, i) => {
                if (!item) return null;
                const config = (item.type && INSIGHT_TYPE_MAP[item.type]) || { color: 'text-secondary font-bold', bg: 'bg-secondary/10', border: 'border-secondary/20', icon: Lightbulb, label: 'Training Insight' };
                      ...
                      <div className="flex-1 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className={`text-[10px] font-black uppercase tracking-widest ${config.color}`}>
                            {config.label} {item.id.startsWith('st-') ? '(Onboarding)' : ''}
                          </span>
                        ...
                        <div className="flex items-center gap-4 pt-2">
                          {!item.is_read && !item.id.startsWith('st-') && (
                            <button 

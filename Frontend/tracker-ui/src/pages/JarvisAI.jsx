    ...
    <div className="flex flex-col h-[calc(100vh-80px)] max-w-6xl mx-auto gap-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2 pb-5">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Jarvis AI Coach</h1>
        ...
        {/* Tab Switcher */}
        <div className="flex gap-1 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-1 w-fit">
          <button
      ...
      {/* ── Body ── */}
      <div className="flex-1 min-h-0 rounded-3xl border border-slate-200 dark:border-white/5 bg-white/90 dark:bg-[#0B1120]/80 backdrop-blur-xl overflow-hidden flex flex-col shadow-lg dark:shadow-[0_8px_60px_rgba(0,0,0,0.6)]">
        ...
        {activeTab === 'chat' && (
          <div className="flex-1 min-h-0 flex flex-col">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-5" ref={scrollRef}
              style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(124,58,237,0.1) transparent' }}>
              
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.role === 'bot' && (
                    <div className="w-9 h-9 rounded-xl bg-[#7C3AED]/10 dark:bg-[#7C3AED]/20 border border-[#7C3AED]/20 dark:border-[#7C3AED]/30 flex items-center justify-center shrink-0 mt-0.5">
                      {msg.loading ? (
                  ...
                  <div className={`flex flex-col gap-1 max-w-[78%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                    <div className={`px-4 py-3.5 rounded-2xl text-sm leading-relaxed font-medium ${
                      msg.role === 'user'
                        ...
                        : 'bg-slate-50 dark:bg-white/[0.04] text-slate-800 dark:text-[#E2E8F0] border border-slate-100 dark:border-white/8 rounded-tl-sm shadow-sm dark:shadow-none'
                    }`}>
                      {msg.loading ? (
                        <span className="flex items-center gap-2 text-slate-400 dark:text-[#94A3B8]">
                          <span className="w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-[#94A3B8] animate-bounce" style={{ animationDelay: '0ms' }} />
                          <span className="w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-[#94A3B8] animate-bounce" style={{ animationDelay: '150ms' }} />
                          <span className="w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-[#94A3B8] animate-bounce" style={{ animationDelay: '300ms' }} />
                        </span>
                      ) : (
                        <span dangerouslySetInnerHTML={{ __html: renderMessage(msg.text) }} />
                      )}
                    </div>
                    <span className="text-[9px] text-slate-500 dark:text-[#475569] font-medium px-1">{formatTime(msg.time)}</span>
                  </div>

                  {msg.role === 'user' && (
                    <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-center justify-center shrink-0 mt-0.5 text-xs font-black text-slate-700 dark:text-white">
              ...
              <div className="px-6 pb-4 grid grid-cols-2 md:grid-cols-4 gap-2">
                {PRESETS.map((p, i) => (
                  <button
                    ...
                    className="flex flex-col items-start gap-1.5 px-4 py-3 rounded-2xl bg-slate-50 dark:bg-white/[0.03] border border-slate-100 dark:border-white/8 hover:bg-[#7C3AED]/5 dark:hover:bg-[#7C3AED]/10 hover:border-[#7C3AED]/20 dark:hover:border-[#7C3AED]/30 transition-all duration-200 group text-left"
                  >
                    <p.icon className="w-4 h-4 text-[#7C3AED] group-hover:scale-110 transition-transform" />
                    <span className="text-[10px] font-black text-slate-500 dark:text-[#94A3B8] uppercase tracking-widest group-hover:text-slate-800 dark:group-hover:text-white transition-colors">{p.label}</span>
                  </button>
                ))}
              </div>
            )}
            ...
            <div className="p-4 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-black/20">
              <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="relative">
                <input
                  ...
                  disabled={isSending}
                  className="w-full bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 focus:border-[#7C3AED]/50 rounded-2xl py-4 pl-5 pr-16 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-[#475569] focus:outline-none transition-all font-medium disabled:opacity-50"
                />
                <button
                  type="submit"
              ...
              <p className="text-[9px] text-center text-slate-500 dark:text-[#334155] mt-2 font-bold uppercase tracking-widest">
          ...
          <div className="flex-1 min-h-0 flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-black/20">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-widest">AI Analysis</span>
                {!insightsLoading && (
                  <span className="text-[9px] font-black text-slate-500 dark:text-[#475569] uppercase tracking-widest ml-2">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-4"
              ...
              {insightsLoading ? (
                <div className="flex flex-col items-center justify-center h-full gap-3 py-20">
                  ...
                  <p className="text-sm text-slate-500 dark:text-[#475569] font-medium">Analyzing your fitness data...</p>
                </div>
              ) : insights.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-4 py-20">
                  ...
                  <div className="text-center">
                    <p className="text-sm font-black text-slate-900 dark:text-white">No Insights Yet</p>
                    <p className="text-xs text-slate-500 dark:text-[#475569] mt-1 max-w-xs">Log a few workouts and health metrics, then click Re-Analyze to generate your personalized insights.</p>
                  </div>
                  <button
                    onClick={handleReAnalyze}
                    disabled={reanalyzing}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1.5">
                          ...
                          <span className="text-[9px] text-slate-500 dark:text-[#475569] font-medium">
                            {new Date(item.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                          </span>
                        </div>
                        <p className="text-sm text-slate-700 dark:text-[#CBD5E1] leading-relaxed font-medium">{item.content}</p>
                        {!item.is_read && (
                          <button
            ...
            <div className="p-4 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-black/20">
              <button
                onClick={() => setActiveTab('chat')}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-100 dark:bg-white/[0.03] border border-slate-200 dark:border-white/8 text-slate-600 dark:text-[#94A3B8] hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/[0.06] text-xs font-black uppercase tracking-widest transition-all"
              >
                <Bot className="w-4 h-4" />

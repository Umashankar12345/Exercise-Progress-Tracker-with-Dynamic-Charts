...
// Premium Overlays
import FloatingAnalyticsCard from '../components/login/FloatingAnalyticsCard';
import SmartwatchConnectedWidget from '../components/login/SmartwatchConnectedWidget';
import LiveActivityFeedOverlay from '../components/login/LiveActivityFeedOverlay';
import SubtleDashboardOverlay from '../components/login/SubtleDashboardOverlay';
...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      {/* Left Side: Visuals */}
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-[#050816]">
        <img 
          src={photography} 
          alt="Fitness Training" 
          className="absolute inset-0 w-full h-full object-cover opacity-80 mix-blend-luminosity"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#050816] via-[#050816]/40 to-transparent z-0" />
        
        <SubtleDashboardOverlay />
        <FloatingAnalyticsCard />
        <SmartwatchConnectedWidget />
        
        <div className="absolute bottom-12 left-12 right-12 z-20 flex flex-col justify-end">
          <div className="mb-8">
            <LiveActivityFeedOverlay />
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                ...
                onClick={(e) => {
                  e.preventDefault();
                  toast.custom((t) => (
                    <div className={`${t.visible ? 'animate-enter' : 'animate-leave'} max-w-md w-full bg-[#0F172A] shadow-2xl shadow-cyan-500/20 rounded-2xl pointer-events-auto flex flex-col p-5 border border-cyan-500/30 backdrop-blur-xl`}>
                      <div className="flex gap-4">
                        <div className="flex-shrink-0 w-12 h-12 bg-cyan-500/20 rounded-full flex items-center justify-center border-2 border-cyan-500/50">
                          <Zap className="h-6 w-6 text-cyan-400" />
                        </div>
                        <div className="flex-1">
                          <p className="text-sm font-bold text-white mb-1">Incoming Transmission</p>
                          <p className="text-sm text-gray-400 font-medium leading-relaxed">
                            Our OAuth relays are currently undergoing a massive matrix upgrade for seamless social integration.
                            <br/><br/>
                            <span className="text-cyan-400 font-bold">Please use manual Sign In or Create an Account to access your dashboard today.</span>
                          </p>
                        </div>
                      </div>
                      <div className="flex justify-end mt-4">
                        <button onClick={() => toast.dismiss(t.id)} className="text-xs font-bold text-white bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg transition-colors">
                          Acknowledge
                        </button>
                      </div>
                    </div>
                ...
                className="flex items-center justify-center gap-2 border border-outline-variant bg-surface-container hover:bg-surface-bright text-sm font-medium text-on-surface py-3 rounded-xl transition-colors"
              >
              ...
              <button 
                ...
                className="flex items-center justify-center gap-2 border border-outline-variant bg-surface-container hover:bg-surface-bright text-sm font-medium text-on-surface py-3 rounded-xl transition-colors"
              >
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.87 8.17 6.84 9.5.5.08.66-.23.66-.5v-1.69c-2.77.6-3.36-1.34-3.36-1.34-.46-1.16-1.11-1.47-1.11-1.47-.9-.62.07-.6.07-.6 1 .07 1.53 1.03 1.53 1.03.9 1.52 2.34 1.07 2.91.83.09-.65.35-1.09.63-1.34-2.22-.25-4.55-1.11-4.55-4.92 0-1.11.38-2 1.03-2.71-.1-.25-.45-1.29.1-2.64 0 0 .84-.27 2.75 1.02.79-.22 1.65-.33 2.5-.33.85 0 1.71.11 2.5.33 1.91-1.29 2.75-1.02 2.75-1.02.55 1.35.2 2.39.1 2.64.65.71 1.03 1.6 1.03 2.71 0 3.82-2.34 4.66-4.57 4.91.36.31.69.92.69 1.85V21c0 .27.16.59.67.5C19.14 20.16 22 16.42 22 12A10 10 0 0012 2z"/></svg>
                <span>GitHub</span>
              </button>
            </div>
          </form>

...
export default function Plan() {
  ...
  const handleDragEnter = (e, index) => {
    ...
    const draggedItem = updatedDays[draggedIdx];
    
    updatedDays.splice(draggedIdx, 1);
    updatedDays.splice(index, 0, draggedItem);
            ...
            <div className="space-y-3">
              {plan.days?.map((day, i) => (
                <div 
                  ...
                  className={`glass-card p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-primary/30 transition-all cursor-grab active:cursor-grabbing group ${
                    draggedIdx === i ? 'opacity-40 border-primary/50 border-dashed bg-primary/5' : ''
                  }`}
                >
                  <div className="flex items-center gap-6">
                    {/* Grab handle indicator */}
                    <div className="text-on-surface-variant/30 group-hover:text-primary/60 transition-colors hidden md:block">
                      <div className="grid grid-cols-2 gap-0.5 w-3.5">
